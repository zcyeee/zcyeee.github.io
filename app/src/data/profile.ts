...
export const experiences: Experience[] = [
  {
    ...
    startDate: "2026-05",
    endDate: "至今",
    intro: "针对混元模型（295B-A21B）在多轮及跨会话场景中的记忆问题，开展上下文理解 RL 训练与记忆消费优化，搭建从线上难例挖掘、Rubric 奖励构造、RL 训练到专项评测与数据飞轮的完整闭环。",
    description: [
      "多轮记忆数据与 Rubric 构建：通过三级清洗产出 1 万余条通用多轮对话；对真实对话轨迹进行多切点采样与判分，构建 2,000 余条记忆专项样本；将记忆能力拆解为信息回溯、指代消解、事实一致等维度，生成 3 万条 Rubric，并通过事实一致性与约束合理性校验保障奖励质量",
      "上下文理解 RLVR 训练：基于细粒度 Rubric 进行 GRPO 训练，通过有效样本筛选提升梯度利用率；结合 Router Replay 与 IcePop 重要性修正缓解训推不一致，并设计融合硬约束门控与记忆 Rubric 评分的门控奖励机制；最终在验证集和短期记忆测试集上分别提升 3pp 和 8pp",
      "真实用户记忆消费评测：基于真实用户跨会话历史，通过 Gold 记忆注入与反事实干扰记忆配对隔离抽取、检索变量，从有效性、选择性与得体性三个层面评估模型能否准确、克制且自然地使用用户记忆",
      "记忆消费数据飞轮：基于消费 Rubric 持续挖掘“记忆漏用、无关信息误用”等 Badcase，采用“自反思修复—强模型兜底重写—Rubric Judge 验收”的分层修复策略构造高质量 SFT 数据，最终在测试集上综合提升 5pp"
    ]
  },
  {
    company: "美团",
    ...
    startDate: "2025-12",
    endDate: "2026-05",
    intro: "针对LLM在复杂环境下多步、多轮工具调用能力不足的问题，基于Qwen模型系统探索多轮Agent任务合成方法与多轮强化学习训练策略，为组内业务模型（35B-A3B）训练积累数据以及实践经验。",
    description: [
