...
category: "工程知识"
excerpt: "配置 Cloudflare R2 作为个人图床，并通过 PicGo 上传图片，最终可生成自定义域名格式的图片链接。"
---

本文记录如何把 Cloudflare R2 配置成个人图床，并通过 PicGo 上传图片，同时上传后自动得到自定义域名格式的图片链接。
...
PicGo 配置 Cloudflare R2 时，主要是把 Cloudflare 中的信息对应到 PicGo 字段：

| 配置项 | Cloudflare 来源 | PicGo 对应字段 | 说明 |
|------|------|------|------|
| Bucket Name | R2 Bucket 名称 | 桶名 | 决定图片上传到哪个存储桶 |
| Account ID | 账户详情 | 间接使用 | 用于确定 S3 Endpoint |
...
Bucket 名称后续会填到 PicGo 的 `桶名` 字段。建议只使用小写字母、数字和短横线。

## Step 2：找到 S3 Endpoint

在 R2 的账户详情中找到 `Account ID`。它通常不需要单独填到 PicGo，而是用来组成 S3 Endpoint：

```text
https://<ACCOUNT_ID>.r2.cloudflarestorage.com
```

如果 Cloudflare 页面直接提供 `S3 API Endpoint`，也可以直接复制。最终填到 PicGo 的 `自定义节点` / `Endpoint` 字段：

```text
https://<ACCOUNT_ID>.r2.cloudflarestorage.com
```

不要在 Endpoint 后面拼接 Bucket 名。Bucket 应单独填到 `桶名` 字段。

## Step 3：创建 R2 API Token

PicGo 上传需要 S3 客户端凭据。进入 `R2 对象存储` 的账户详情，管理 API Token，并创建一组 R2 凭据。
...
在 PicGo 的 Amazon S3 / S3 插件中，按下面填写：
