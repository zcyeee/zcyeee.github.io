...
> **重要性采样让旧策略采集的数据可以用于估计新策略的目标，但当 $r_t(\theta)$ 偏离 1 太远时，估计方差会变大，训练也会变得不稳定。**
...
这里取 $\min$ 而非直接裁剪 $r_t$ 是关键：它让裁剪只在 $r_t(\theta)$ 朝着"使目标继续变好"的方向越界时才生效，而在反方向（纠正已经偏离过头的概率）上不做限制。下面两节按优势符号分别展开，可以看到这一非对称行为的具体来源。
